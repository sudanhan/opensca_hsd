...
throw (
	...
	if (!foundDev) {
		// 设备节点是一个类（包含设备指针，设备管理器指针，设备ID，设备的软件描述文件）
		// 设备向域注册是通过m_devNodes实现的，而设备管理器向域注册是通过m_devMgrs实现的
...
throw (
	...
	// 设备管理器注册的重要操作，要将设备管理器的文件系统加到域的文件管理器中
	try {
