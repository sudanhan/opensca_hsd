...
// 基类前面可以有访问说明符，表明ApplicationFactory_impl只能访问POA_CF::ApplicationFactory的public成员
// virtual表示虚继承，确保内存中只存在基类成员的一份拷贝
class ApplicationFactory_impl :
	public virtual POA_CF::ApplicationFactory {

  // public是访问说明符，定义在pulic说明符的成员在整个程序内可被访问，
  public:
	// 对变量的声明，该变量的类型为指向DeployStrategy类的指针
	DeployStrategy * m_DelpoyStrategy;

	// 对构造函数的声明
	ApplicationFactory_impl(
		//const的含义表示不能通过sftwpfl改变其所指对象的值，即不能对*sftwpfl赋值
	    const char * sftwpfl,
		// CF::DomainManager::ApplicationSequence是一个类，也是自定义的一种数据结构
	 ...
	 */
	// create函数的声明
	// 函数的定义和函数的声明唯一的区别是函数声明无须函数体，用一个分号替代即可
	CF::Application_ptr
	create(
	    const char * name,
	...
	identifier()
	{
		// c_str是封装在string类中的一个函数，返回当前字符串的首字符地址，为const char*类型
		// 标准头文件<cstring>包含操作c-串的函数库当调用库函数
		// 客户程序提供的是string类型参数，而库函数内部实现用的是c-串，因此需要将string对象，转化为char*对象
		// 返回的是满足corba格式的字符类型？（我这样理解）
	...
	// 友元，AppFactoryEventHandler可以访问ApplicationFactory_impl的私有成员
  ...
  // private是访问说明符，定义在private后面的成员可以被类的成员函数访问，但是不能被使用该类的代码访问
  private:
	...
	// vector是标准库类型，UsesConnectionInfo *表示vector中对象的类型
